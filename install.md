...
13. *If step 9-12 doesn't work, exit the failed server using ```Command + C``` or ```Ctrl + C```, use the command ``cd backend``, then ```python```, then copy all contents in the ```backend/Contact_Scraper.ipynb``` file into the terminal, press Enter twice to run. Once done, type ```exit()``` to get out of the python function.*
...
6. *Again, if the server cannot be accessed, exit it using ```Command + C``` or ```Ctrl + C```, use the command ``cd backend``, then ```python```, then copy the code you want to run in the ```backend/Finance_Scraper.ipynb``` file into the terminal, press Enter twice to run. Once done, type ```exit()``` to get out of the python function.*
